...
function StatsBar({ tournaments, leaderboard }) {
  ...
  const stats = [
    { label: 'Registered Players', value: totalPlayers,  suffix: '+',  color: '#00f5ff' },
    ...
    <section className="py-24 relative">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <Link to="/leaderboard" className="font-display text-sm text-[#4a5568] hover:text-[#00f5ff] transition-colors tracking-wider uppercase">
            View All →
          </Link>
        </motion.div>
        ...
        {loading ? (
          <div className="space-y-3">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="h-16 border border-[#1a2545] animate-pulse bg-[#0a0f1e]/40" />
            ))}
          </div>
          ...
          <div className="text-center py-16 border border-[#1a2545]/40 border-dashed">
            ...
            <Link to="/tournaments" className="inline-block mt-4 font-mono text-sm text-[#00f5ff] hover:text-white transition-colors">
              Browse Tournaments →
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
            {leaderboard.slice(0, 5).map((player, i) => (
              <motion.div key={player.id || i}
                ...
                <div className="w-8 font-display font-bold text-xl"
                ...
                <div className="flex-1">
                  <div className="font-display font-semibold text-white group-hover:text-[#00f5ff] transition-colors">
                    {player.username}
                  </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
          ...
          <h2 className="font-display font-bold text-5xl text-white mb-4">
            GHQ <span className="text-[#ffd700]" style={{ textShadow:'0 0 30px #ffd70066' }}>COIN</span> ECONOMY
          </h2>
          <p className="text-[#4a5568] font-body max-w-xl mx-auto">
            Every action earns. Every win rewards. GHQ Coins power the entire platform.
          </p>
        </motion.div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {coinTypes.map((c, i) => (
            <motion.div key={c.label}
              ...
              className="p-4 border bg-[#0a0f1e]/60 group hover:bg-[#0a0f1e] transition-colors"
...
export default function Home() {
  ...
  // Filter tournaments for homepage section
  ...
  // Show only live + upcoming on homepage (not completed/cancelled)
  const activeOnly = filtered.filter(t => {
    const status = (t.status || '').toLowerCase()
    return status === 'live' || status === 'upcoming' || filter !== 'all'
  })
    ...
    <div className="relative min-h-screen">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }}
          ...
          <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.7 }}
            ...
            <Link to="/tournaments"
              ...
              <span className="relative font-display font-bold text-base tracking-widest uppercase text-[#050810]">
                {hasAnyTournaments ? 'Browse Tournaments' : 'View Tournaments'}
              </span>
            </Link>
            {!isLoggedIn && (
              <Link to="/register"
            ...
            {isLoggedIn && (
              <Link to="/wallet"
                className="relative px-8 py-4 overflow-hidden group border border-[#f5a623]/40 hover:border-[#f5a623]/80 transition-colors"
                style={{ clipPath:'polygon(16px 0, 100% 0, 100% 100%, 0 100%, 0 16px)' }}>
                <span className="font-display font-bold text-base tracking-widest uppercase text-[#f5a623]">
                  🪙 Buy Gollars
                </span>
              </Link>
            )}
          ...
          {/* Live stat pills — real data */}
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:1.0 }}
            className="flex flex-wrap justify-center gap-4 mt-12">
            {[
              ['🏆', tournaments.length.toString(),                 'Tournaments'],
              ['🔴', liveCount > 0 ? liveCount.toString() : '0',   'Live Now'],
              ['👥', leaderboard.length.toString(),                 'Players'],
            ].map(([icon, val, label]) => (
        ...
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
            ...
            <div>
              ...
              <h2 className="font-display font-bold text-5xl text-white">
                ACTIVE <span className="text-[#00f5ff]">ARENA</span>
              </h2>
            </div>
          ...
          {loading && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="h-72 border border-[#1a2545] animate-pulse bg-[#0a0f1e]/40"
          ...
          {!loading && activeOnly.length === 0 && (
            <div className="text-center py-24 border border-[#1a2545]/40 border-dashed">
              ...
              <p className="font-mono text-sm text-[#4a5568] max-w-md mx-auto mb-6">
                {filter === 'all'
                  ? 'We\'re setting up the arena. Register now to be notified when the first tournament drops!'
                  : 'Try a different filter or check back soon.'}
              </p>
              {!isLoggedIn && filter === 'all' && (
                <Link to="/register"
                  className="inline-block px-6 py-3 bg-[#00f5ff] font-display font-bold text-sm tracking-widest uppercase text-[#050810] hover:bg-[#00f5ff]/85 transition-colors">
                  Register Free →
                </Link>
              )}
            </div>
      ...
      <section className="py-20 relative border-y border-[#1a2545]/40">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
            ...
            <h2 className="font-display font-bold text-5xl text-white">
              TWO PATHS TO <span className="text-[#ffd700]">GLORY</span>
            </h2>
          </motion.div>
          <div className="grid md:grid-cols-2 gap-6">
            <motion.div initial={{ opacity:0, x:-30 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}
              ...
              <p className="text-[#4a5568] font-body leading-relaxed mb-6">Open format for all players. Free or paid entry. Compete for GHQ Coins and prize pools. Perfect for all skill levels.</p>
              <div className="space-y-2">
                {['Free or Paid entry','Open to all skill levels','GHQ Coins for participation','Prize pool distribution'].map(f => (
            ...
            <motion.div initial={{ opacity:0, x:30 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}
              ...
              <h3 className="font-display font-bold text-3xl text-white mb-3">CHAMPIONS 👑</h3>
              <p className="text-[#4a5568] font-body leading-relaxed mb-6">Elite bracket tournaments. Always paid. Single or double elimination. Highest stakes, biggest rewards, exclusive titles.</p>
              <div className="space-y-2">
                {['Paid entry only','Bracket elimination system','Massive prize pools','Exclusive Champion badge'].map(f => (
                  <div key={f} className="flex items-center gap-2 font-body text-sm text-[#e8eaf6]/70">
      ...
      <section className="py-32 relative overflow-hidden">
        <div className="absolute inset-0"
          style={{ background:'radial-gradient(ellipse at center, #00f5ff08 0%, transparent 70%)' }} />
        <HexGrid />
        <div className="relative max-w-3xl mx-auto px-6 text-center">
          <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}>
            ...
            <h2 className="font-display font-bold text-6xl text-white mb-6 leading-none">
              READY TO<br/>
              <span className="text-[#00f5ff]" style={{ textShadow:'0 0 40px #00f5ff66' }}>DOMINATE?</span>
            </h2>
            <p className="text-[#4a5568] font-body mb-10">
              {isLoggedIn
                ? `Welcome back, ${user?.username}! Jump into a tournament and start earning.`
                : 'Register free. Earn GHQ Coins. Rise through the ranks. Claim your glory.'}
            </p>
